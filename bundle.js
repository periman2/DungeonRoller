...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _player_move_action = __webpack_require__(201);

	var _create_level_action = __webpack_require__(202);

	var _alter_level_action = __webpack_require__(203);

	var _move_screen_action = __webpack_require__(204);
	...
	var Game = function (_Component) {
	    ...
	    function Game(props) {
	        ...
	        _this.renderPosition = _this.renderPosition.bind(_this);
	        _this.handleKeyDown = _this.handleKeyDown.bind(_this);
	        _this.buildRooms = _this.buildRooms.bind(_this);
	        _this.checkOverlap = _this.checkOverlap.bind(_this);
	        _this.getAllCorners = _this.getAllCorners.bind(_this);
	        _this.findIfNotInside = _this.findIfNotInside.bind(_this);
	        _this.buildPaths = _this.buildPaths.bind(_this);
	        _this.findConnectedRooms = _this.findConnectedRooms.bind(_this);
	        _this.makeCorridors = _this.makeCorridors.bind(_this);
	        _this.mergeCorridors = _this.mergeCorridors.bind(_this);
	        _this.getInitialPosition = _this.getInitialPosition.bind(_this);
	        _this.getNeighbors = _this.getNeighbors.bind(_this);
	        _this.addElementsAndWalls = _this.addElementsAndWalls.bind(_this);
	        _this.createBeginning = _this.createBeginning.bind(_this);
	        _this.drawShrines = _this.drawShrines.bind(_this);
	    ...
	    _createClass(Game, [{
	        ...
	        value: function renderPosition() {
	            ...
	            var fill = this.props.player.elem.texture;
	        ...
	        key: 'buildRooms',
	        value: function buildRooms(levelIndex) {
	            var ctx = this.getCanvas();
	            if (!ctx) {
	                return null;
	            }
	            ...
	            var density = this.props.gameInfo.roomDensity;
	            for (var i = 0; i < density; i++) {
	                ...
	                if (rooms.length > 0) {
	                    var isOverlap = this.checkOverlap(newRoom, rooms, width, height);
	                    if (isOverlap) {
	        ...
	        key: 'drawShrines',
	        value: function drawShrines(room) {
	            var shrines = room.shrines;
	            if (shrines.length > 0) {
	        ...
	        key: 'makeRoomAttributes',
	        value: function makeRoomAttributes(room, index, roomType) {
	            room.index = index;
	            room.type = roomType;
	            room.traps = [];
	            room.shrines = [];
	        ...
	        key: 'addElementsAndWalls',
	        value: function addElementsAndWalls(leftIndex, level, levelIndex) {
	            // console.log(leftIndex, level);
	            var rooms = level.rooms;
	            var randomRooms = rooms.reduce(function (prevRoom, r) {
	            ...
	            for (var i = 0; i < randomRooms.length; i++) {
	                var room = randomRooms[i];
	                var neighbors = this.getNeighbors(room, level.paths, rooms);
	                ...
	                var trapsAndElement = this.makeSpecialTraps(room);
	                var traps = trapsAndElement[0];
	                var elemPos = trapsAndElement[1];
	                var elem = this.chooseElem(type, elemPos, this.props.elements);
	                room.elements.push(elem);
	                room.traps.push(traps);
	                for (var j = 0; j < neighbors.length; j++) {
	                    var wall = this.makeWalls(neighbors[j], room, this.props.gameInfo.radius);
	        ...
	        key: 'chooseElem',
	        value: function chooseElem(type, elem, allElements) {
	            switch (type.name) {
	                case 'rockWall':
	        ...
	        key: 'makeSpecialTraps',
	        value: function makeSpecialTraps(room) {
	            var randNum = 0.62 - Math.random() * 0.22;
	            var reverseRandNum = 1 - randNum;
	            var rad = this.props.gameInfo.radius;
	            if (Math.random() > 0.5) {
	                var trap1 = {
	                ...
	                var rand = rad * 2 + rad * 3 * Math.random();
	                ...
	                _trap = this.getAllCorners(_trap);
	                var _trap2 = {
	                    X: _trap.X,
	                    Y: _trap.Y + _trap.boxHeight + rad * 2,
	                    boxWidth: _trap.boxWidth,
	                    boxHeight: reverseRandNum * room.boxHeight * 0.6 - rad
	                };
	                _trap2 = this.getAllCorners(_trap2);
	                var _trap3 = {
	                    X: _trap.X + _trap.boxWidth,
	                    Y: _trap.Y,
	                    boxWidth: rand,
	                    boxHeight: room.boxHeight * 0.6
	                };
	                _trap3 = this.getAllCorners(_trap3);
	                var _element = {
	                    X: _trap3.X - rad * 2,
	                    Y: _trap.Y + _trap.boxHeight,
	                    boxHeight: rad * 2,
	                    boxWidth: rad * 2
	                };
	                _element = this.getAllCorners(_element);
	                return [[_trap, _trap2, _trap3], _element];
	            }
	        }
	    }, {
	        key: 'drawWall',
	        ...
	        key: 'chooseWallType',
	        value: function chooseWallType(index) {
	            var type = {};
	            switch (index) {
	                case 1:
	        ...
	        key: 'makeWalls',
	        value: function makeWalls(neighbor, room, radius) {
	            var newWall = {};
	            var nC = neighbor.corners;
	        ...
	        key: 'createBeginning',
	        value: function createBeginning(room, levelIndex) {
	            var height = room.boxHeight;
	            var width = room.boxWidth;
	            var shrines = [];
	            var shrine_height = 0.8 * height / levelIndex;
	        ...
	        key: 'choseShrineColor',
	        value: function choseShrineColor(shrine) {
	            switch (shrine.type) {
	                case 'fire':
	                    if (shrine.active) {
	        ...
	        value: function getInitialPosition(level) {
	            ...
	            var X = leftRoom.X + leftRoom.boxWidth / 2;
	            var Y = leftRoom.Y + leftRoom.boxHeight / 2;
	            var rad = this.props.gameInfo.radius;
	        ...
	        key: 'buildPaths',
	        value: function buildPaths(rooms, width, height) {
	            var corridorsH = [];
	            var corridorsV = [];
	            var coredorWidth = this.props.gameInfo.coredorWidth;
	            var divW = Math.floor(width / coredorWidth);
	            var divH = Math.floor(height / coredorWidth);
	            ...
	            for (var i = 0; i <= divH; i++) {
	                ...
	                var findConnectionsH = this.findConnectedRooms(rangeH, rooms, 'Y');
	            ...
	            for (var j = 0; j <= divW; j++) {
	                ...
	                var findConnectionsV = this.findConnectedRooms(rangeV, rooms, 'X');
	            ...
	            var allCorredors = corridorsH.concat(corridorsV);
	        ...
	        key: 'mergeCorridors',
	        value: function mergeCorridors(cor, orient) {
	            var newCor = cor;
	            var updated = [];
	                ...
	                for (var j = 0; j < i; j++) {
	                    ...
	                    if (newCor[i].connects.indexOf(newCor[j].connects[0]) !== -1 && newCor[i].connects.indexOf(newCor[j].connects[1]) !== -1) {
	                        if (orient === 'Y') {
	                            ...
	                            var rand = Math.random() * newCor[j].boxHeight;
	                            ...
	                            var _rand = Math.random() * newCor[j].boxWidth;
	                            newCor[i].boxWidth += _rand;
	            ...
	            newCor = newCor.filter(function (el) {
	                return el;
	            });
	            // console.log(cor);
	            return newCor;
	        ...
	        key: 'findConnectedRooms',
	        value: function findConnectedRooms(range, rooms, orient) {
	            var foundRooms = [];
	            for (var i = 0; i < rooms.length; i++) {
	                var rC1 = rooms[i].corners.UL[orient];
	                var secCorner = orient === 'X' ? 'UR' : 'DL';
	                var rC2 = rooms[i].corners[secCorner][orient];
	                var minDist = 0;
	                var logic = false;
	                if (orient === 'Y') {
	            ...
	            var corridors = this.makeCorridors(foundRooms, range, orient);
	            return corridors;
	        }
	    }, {
	        key: 'sortFoundRooms',
	        value: function sortFoundRooms(foundRooms, orient) {
	            if (orient === 'Y') {
	        ...
	        key: 'makeCorridors',
	        value: function makeCorridors(foundRooms, range, orient) {
	            var corridors = [];
	            if (foundRooms.length > 1) {
	                for (var j = 1; j < foundRooms.length; j++) {
	                    var logic = false;
	                    if (orient === 'X') {
	        ...
	        key: 'drawBox',
	        value: function drawBox(box, color) {
	            var ctx = this.getCanvas();
	            var img = new Image();
	            if (!ctx) {
	                return null;
	        ...
	        key: 'checkOverlap',
	        value: function checkOverlap(newRoom, rooms, width, height) {
	            var cornersOfNewRoom = this.getAllCorners(newRoom);
	            if (cornersOfNewRoom.corners.UR.X > width | cornersOfNewRoom.corners.DL.Y > height) {
	                return false;
	            }
	            var NotOverlap = false;
	            for (var j = 0; j < rooms.length; j++) {
	                var cornersEl = this.getAllCorners(rooms[j]);
	        ...
	        key: 'findIfNotInside',
	        value: function findIfNotInside(room1, room2) {
	            var c = room1.corners;
	            var c2 = room2.corners;
	            var lineChecker = this.checkForLines(c, c2);
	            if (lineChecker === false) {
	                return false;
	            }
	            var lineChecker2 = this.checkForLines(c2, c);
	            if (lineChecker2 === false) {
	                return false;
	            }
	            for (var corner in c) {
	                var checker = this.checkOneCorner(c, c2, corner);
	                if (checker === false) {
	                    return false;
	                }
	                var checker2 = this.checkOneCorner(c2, c, corner);
	                if (checker2 === false) {
	                    return false;
	                }
	            }
	        ...
	        key: 'checkForLines',
	        value: function checkForLines(c, c2) {
	        ...
	        key: 'checkOneCorner',
	        value: function checkOneCorner(c, c2, corner) {
	        ...
	        key: 'getAllCorners',
	        value: function getAllCorners(room) {
	            var corners = {};
	        ...
	        key: 'getRandomDimention',
	        value: function getRandomDimention(dim, object) {
	            if (object === 'start') {
	                return Math.floor(Math.random() * dim);
	            }
	            var newRoomDim = Math.ceil((0.09 + Math.random() * 0.1) * dim);
	            return newRoomDim;
	        }
	    }, {
	        key: 'getCanvas',
	        ...
	        key: 'getNeighbors',
	        value: function getNeighbors(location, paths, rooms) {
	            var connected = [];

	            if (location.type === 'room') {
	                connected = paths.filter(function (cor) {
	                    return cor.connects.indexOf(location.index) !== -1;
	                });
	            } else {
	                // console.log('location: ', location)
	                var neightborCor = paths.filter(function (cor) {
	                    if (this.checkOverlappingCorredors(cor, location) === true || this.checkOverlappingCorredors(location, cor) === true) {
	                        return true;
	                    }
	                    return false;
	                }.bind(this));
	                var neighborRooms = rooms.filter(function (room) {
	        ...
	        key: 'checkOverlappingCorredors',
	        value: function checkOverlappingCorredors(cor1, cor2) {
	            var c = cor1.corners;
	            var c2 = cor2.corners;
	            if (c.DL.X > c2.DL.X && c.DR.X < c2.DR.X && c2.UL.Y > c.UL.Y && c2.DL.Y < c.DL.Y) {
	                return true;
	            ...
	            if (this.props.player.velocity > 0 && this.props.player.life > 0) {
	                ...
	                var animateBall = function animateBall() {
	                    ...
	                    if (currentPlace.type === 'room') {
	                        if (currentPlace.shrines.length > 0) {
	                            ...
	                            if (finished) {
	                                ...
	                                var newLevel = levels.filter(function (l) {
	                    ...
	                    if (_this2.props.player.velocity === 0) {
	                        ...
	                        _this2.props.changePlayerInfo(action2);
	                    }
	                    if (_this2.props.player.velocity === _this2.props.player.initialVel) {
	                        cancelAnimationFrame(myFrame);
	                    }
	            ...
	            if (currentPlace.type === 'room') {
	                if (currentPlace.shrines.length > 0) {
	                    currentPlace.shrines.map(function (s) {
	                        {
	                            _this3.drawBox(s, _this3.choseShrineColor(s));
	                        }
	                    });
	                }
	        ...
	        value: function render() {
	            this.renderPosition();
	            return _react2.default.createElement('canvas', { id: 'mycanvas', height: this.props.gameInfo.canvasHeight, width: this.props.gameInfo.canvasWidth });
	        }
	    }]);
	...
	function mapDispatchToProps(dispatch) {
	    return (0, _redux.bindActionCreators)({
	        changePlayerInfo: _player_move_action.changePlayerInfo,
	        makeNewLevel: _create_level_action.makeNewLevel,
	        changeLevels: _alter_level_action.changeLevels,
	        moveScreen: _move_screen_action.MoveScreen
	    }, dispatch);
	};
...
/***/ (function(module, exports) {
	...
	exports.changePlayerInfo = changePlayerInfo;
	var PLAYER_INFO_CHANGE = exports.PLAYER_INFO_CHANGE = 'PLAYER_INFO_CHANGE';

	function changePlayerInfo(info) {
	    return {
...
/***/ (function(module, exports) {
	...
	exports.makeNewLevel = makeNewLevel;
	var MAKE_LEVEL = exports.MAKE_LEVEL = 'MAKE_LEVEL';

	function makeNewLevel(level) {
	    return {
	        type: MAKE_LEVEL,
	        payload: level
	    };
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _gameInfo = __webpack_require__(207);

	var _gameInfo2 = _interopRequireDefault(_gameInfo);

	var _reducer_player_info = __webpack_require__(208);

	var _reducer_player_info2 = _interopRequireDefault(_reducer_player_info);

	var _reducer_new_level = __webpack_require__(209);

	var _reducer_new_level2 = _interopRequireDefault(_reducer_new_level);

	var _reducer_move_screen = __webpack_require__(210);

	var _reducer_move_screen2 = _interopRequireDefault(_reducer_move_screen);

	var _reducer_elements = __webpack_require__(211);

	var _reducer_elements2 = _interopRequireDefault(_reducer_elements);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	var rootReducer = (0, _redux.combineReducers)({
	  gameInfo: _gameInfo2.default,
	  player: _reducer_player_info2.default,
	  levels: _reducer_new_level2.default,
	  elements: _reducer_elements2.default,
	  screenLocation: _reducer_move_screen2.default
	});
...
/***/ (function(module, exports) {
	...
	exports.default = function () {
	    return {
	        ...
	        coredorWidth: 19,
	        roomDensity: 2000,
	        roomColor: 'https://dl.dropboxusercontent.com/s/jm3h1hvw32l7ur5/floor2.png?dl=0',
	        corredorColor: 'https://dl.dropboxusercontent.com/s/jm3h1hvw32l7ur5/floor2.png?dl=0',
	        radius: 6,
	        initialVel: 1.3,
...
/***/ (function(module, exports, __webpack_require__) {
	...
	exports.default = function () {
	    ...
	    switch (action.type) {
	        case _player_move_action.PLAYER_INFO_CHANGE:
	            var player = positionEdit(action, state);
	            return player;
	    }
	    return state;
	};
	...
	function positionEdit(action, state) {
	    ...
	    if (state.position && action.payload.currentPlace) {
	        ...
	        var neighbors = getNeighbors(currentPlace, level.paths, level.rooms);
	        var vel = player.velocity;
	        var initialVel = action.payload.initialVel;
	        if (vel > 0) {
	...
	function handleCollision(player, newPos, currentPlace, neighbors) {
	    var playerBox = makeCorners(newPos, player.radius);
	    var collision = {
	        collided: true,
	    ...
	    neighbors = neighbors.map(function (n) {
	        if (n.type === 'room') {
	            if (n.walls.length > 0) {
	                n.walls = n.walls.filter(function (w) {
	                    // console.log('-------------------------this is a wall', w);
	                    if (checkOneCorner(playerBox.UL, w.corners) || checkOneCorner(playerBox.UR, w.corners) || checkOneCorner(playerBox.DR, w.corners) || checkOneCorner(playerBox.DL, w.corners)) {
	    ...
	    if (currentPlace.walls) {
	        ...
	        currentPlace.walls = currentPlace.walls.reduce(function (prevW, w) {
	            if (checkOneCorner(playerBox.UL, w.corners) || checkOneCorner(playerBox.UR, w.corners) || checkOneCorner(playerBox.DR, w.corners) || checkOneCorner(playerBox.DL, w.corners)) {
	    ...
	    if (currentPlace.type === 'room') {
	        ...
	        if (currentPlace.traps.length > 0) {
	            currentPlace.traps[0].map(function (t) {
	        ...
	        if (currentPlace.elements.length > 0) {
	            currentPlace.elements.map(function (e) {
	        ...
	        if (currentPlace.shrines.length > 0) {
	            currentPlace.shrines = currentPlace.shrines.map(function (s) {
	    ...
	    if (playerBox.UL.X >= currentPlace.corners.UL.X && playerBox.UR.X <= currentPlace.corners.UR.X && playerBox.UL.Y >= currentPlace.corners.UL.Y && playerBox.DL.Y <= currentPlace.corners.DL.Y) {
	        collision.collided = false;
	        return collision;
	    }
	    var cP = currentPlace.corners;
	    var inNeighbor = neighbors.filter(function (neighbor) {
	...
	function checkPair(playerBox, nC, c1, c2) {
	    if (playerBox[c1].X < nC.UR.X && playerBox[c1].X > nC.UL.X && playerBox[c1].Y > nC.UL.Y && playerBox[c1].Y < nC.DL.Y && playerBox[c2].X < nC.UR.X && playerBox[c2].X > nC.UL.X && playerBox[c2].Y > nC.UL.Y && playerBox[c2].Y < nC.DL.Y) {
	        return true;
	    }
	    return false;
	}
	...
	function checkOne(pos, currentPlace) {
	    if (pos.x < currentPlace.corners.UR.X && pos.x > currentPlace.corners.UL.X && pos.y > currentPlace.corners.UL.Y && pos.y < currentPlace.corners.DL.Y) {
	        return false;
	    }
	    return true;
	}
	...
	function checkOneCorner(corner, corners) {
	    if (corner.X < corners.UR.X && corner.X > corners.UL.X && corner.Y > corners.UL.Y && corner.Y < corners.DL.Y) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	function NewLevel() {
	    ...
	    switch (action.type) {
	        case _create_level_action.MAKE_LEVEL:
	            return [action.payload].concat(_toConsumableArray(state));
	        case _alter_level_action.CHANGE_LEVEL:
...
/***/ (function(module, exports, __webpack_require__) {
	...
	exports.default = moveScreen;

	var _move_screen_action = __webpack_require__(204);

	function moveScreen() {
	    var state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { x: 0, y: 0 };
	    var action = arguments[1];

	    switch (action.type) {
	        case _move_screen_action.MOVE_SCREEN:
	            return action.payload;
	    }
	    return state;
	}
